<?php
// $Id: criteria.php,v 1.12.24.5 2005/07/26 07:11:46 mithyt2 Exp $
//  ------------------------------------------------------------------------ //
...
class CriteriaCompo extends CriteriaElement
{
    ...
    /**
     * Make the criteria into a insert sql string
     *
     * @return  string
     */
    function insertsql()
    {
        ...
        if ($count > 0) {
            $ret = "(". $this->criteriaElements[0]->column;
            for ($i = 1; $i < $count; $i++) {
                $ret .= ", ".$this->criteriaElements[$i]->column;
            }
            $ret .= ") VALUES (";
            $ret .= "'".$this->criteriaElements[0]->value."'";
            for ($i = 1; $i < $count; $i++) {
                $ret .= ", '".$this->criteriaElements[$i]->value."'";
            }
            $ret .= ")";
        }
        return $ret;
    ...
    /**
     * Make the criteria into a update sql string
     *
     * @return  string
     */
    function updatesql()
    {
        ...
        $count = count($this->criteriaElements);
        if ($count > 0) {
            $ret = "SET ".$this->criteriaElements[0]->column."='".$this->criteriaElements[0]->value."'";
            for ($i = 1; $i < $count; $i++) {
                $ret .= ", ".$this->criteriaElements[$i]->column."='".$this->criteriaElements[$i]->value."'";
            }
        }
        return $ret;
    }
...
class Criteria extends CriteriaElement
{
    ...
    function Criteria($column, $value='', $operator='=', $prefix = '', $function = '') {
        ...
        if ($check === false) {
            ...
            $column = substr($column, 0, $check + 1) . "`" . substr($column, $check + 1) . "`";
        }
