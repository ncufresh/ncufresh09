...
$fdata = $currdb -> query("SELECT * FROM `".$currdb -> prefix("news_upfile")."` WHERE news_no IN (".implode(', ', $files).")");
$files = array();
while ($f = $currdb->fetch_array($fdata)) {
    $files[$f['news_no']][] = $f;
}

foreach ($news_dis as $k=>$v) {
    $v['files'] = $files[$v['news_no']];
    $news_dis[$k] = $v;
}
